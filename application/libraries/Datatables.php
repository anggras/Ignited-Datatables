...
class Datatables {
    ...
    private function get_ordering() {
        if ($this->check_column_data())
            $mColArray = $this->get_column_data();
        else
            $mColArray = $this->columns;

        $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
        $columnsToShow = array_values(array_diff($this->columns, $this->unset_columns));

        $orders = $this->ci->input->get_post('order');
        $columns = $this->ci->input->get_post('columns');
        for ($i = 0; $i < count($orders); $i++)
            if (isset($mColArray[intval($orders[$i]['column'])]) && in_array($mColArray[intval($orders[$i]['column'])], $columnsToShow) && $columns[intval($orders[$i]['column'])]['orderable'] == 'true')
                $this->ci->db->order_by($mColArray[intval($orders[$i]['column'])], $orders[$i]['dir']);
    }
    ...
    private function get_filtering() {
        ...
        $sWhere = '';
        $globalSearch = $this->ci->input->get_post('search');
        $sSearch = $this->ci->db->escape_like_str(trim($globalSearch['value']));
        $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
        $columnsToShow = array_values(array_diff($this->columns, $this->unset_columns));
        ...
        if ($sSearch != '')
            for ($i = 0; $i < count($mColArray); $i++)
                if ($columns[$i]['searchable'] == 'true' && in_array($mColArray[$i], $columnsToShow))
        ...
        $sRangeSeparator = $this->ci->input->get_post('sRangeSeparator');

        for ($i = 0; $i < count($columns); $i++) {
            if (isset($columns[$i]['search']['value']) && $columns[$i]['search']['value'] != '' && in_array($mColArray[$i], $columnsToShow)) {
                $miSearch = explode(',', $columns[$i]['search']['value']);
    ...
    private function produce_output($output, $charset) {
        ...
        foreach ($rResult->result_array() as $row_key => $row_val) {
            $aaData[$row_key] = ($this->check_column_data()) ? $row_val : array_values($row_val);
            ...
            foreach ($this->edit_columns as $modkey => $modval)
                foreach ($modval as $val)
                    $aaData[$row_key][($this->check_column_data()) ? $modkey : array_search($modkey, $this->columns)] = $this->exec_replace($val, $aaData[$row_key]);

            $aaData[$row_key] = array_diff_key($aaData[$row_key], ($this->check_column_data()) ? $this->unset_columns : array_intersect($this->columns, $this->unset_columns));
            ...
            foreach ($custom_val['replacement'] as $key => $val) {
                ...
                if (preg_match('/(\w+::\w+|\w+)\((.*)\)/i', $val, $matches) && is_callable($matches[1])) {
                    ...
                    foreach ($args as $args_key => $args_val) {
                        ...
                        $args[$args_key] = (in_array($args_val, $this->columns)) ? ($row_data[($this->check_column_data()) ? $args_val : array_search($args_val, $this->columns)]) : $args_val;
                    }

                    $replace_string = call_user_func_array($func, $args);
                } elseif (in_array($sval, $this->columns))
                    $replace_string = $row_data[($this->check_column_data()) ? $sval : array_search($sval, $this->columns)];
                else
                    $replace_string = $sval;
     ...
     * @return bool
     */
    private function check_column_data() {
        $columns = $this->ci->input->get_post('columns');

        for ($i = 0; $i < count($columns); $i++)
            if (!is_numeric($columns[$i]['data']))
                return TRUE;
